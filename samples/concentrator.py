...
{{
  "concentrated_knowledge": "строка — извлечённая методичка, РЕЛЕВАНТНАЯ конкретному вопросу. Сохраняйте дословно конкретные пороги, таблицы решений, названия методов (Resource Smoothing, Leveling, Crashing, Fast Tracking и пр.) и пошаговые инструкции. Убирайте нерелевантные разделы. Цель — сжатие в 3-10 раз по сравнению с оригиналом.",
...
1. Имена инструментов и widget intents УЖЕ есть в методичке выше. НЕ выдумывайте новые — берите ТОЛЬКО из того, что написано в скиллах.
   **ВАЖНОЕ РАЗЛИЧЕНИЕ:**
   - `planned_tools` — это ИМЕНА ИНСТРУМЕНТОВ, они перечислены в разделе «Используемые MCP инструменты» каждого скилла.
   - `planned_widgets[].intent` — это ИМЕНА INTENTS ВИДЖЕТОВ. Они перечислены в YAML-блоке `available_widgets` под полем `name` у каждого intent-а.
