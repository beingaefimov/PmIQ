...
Ваша задача: прочитать методички (приведены ниже полностью) и извлечь ТОЛЬКО то, что нужно для ответа на ВОПРОС ПОЛЬЗОВАТЕЛЯ. Сформируйте самодостаточный документ-план, который заменит исходную методичку в следующем цикле ReAct.

ВОПРОС ПОЛЬЗОВАТЕЛЯ:
{query}

МЕТОДИЧКИ (читайте внимательно — инструменты, виджеты, пороги — всё внутри):
{skills_content}

ВЫВОД — строгий JSON, без markdown-ограждений, без пояснений:
{{
  "concentrated_knowledge": "строка — извлечённая методичка, РЕЛЕВАНТНАЯ конкретному ВОПРОСУ ПОЛЬЗОВАТЕЛЯ. Сохраняйте дословно конкретные пороги, таблицы решений, названия методов (Resource Smoothing, Leveling, Crashing, Fast Tracking и пр.) и пошаговые инструкции. Убирайте нерелевантные разделы. Цель — сжатие методички в 3-10 раз по сравнению с оригиналом.",
  "planned_tools": ["имя_инструмента_1", "имя_инструмента_2"],
  "planned_widgets": [
    {{
      "intent": "точное_имя_intent_из_метоичек_выше",
      "widget_type": "тип_как_объявлено_в_yaml_методички",
...
КРИТИЧЕСКИЕ ПРАВИЛА:
1. Имена инструментов и widget intents УЖЕ есть в методичках выше. НЕ выдумывайте новые — берите ТОЛЬКО из того, что написано в методичках.
   **ВАЖНОЕ РАЗЛИЧЕНИЕ:**
   - например `planned_tools` — это ИМЯ ИНСТРУМЕНТА, имена инструментов перечислены в разделе «Required Tools» методички.
   - например `planned_widgets[].intent` — это ИМЯ INTENTS ВИДЖЕТА. Имена intents иджетов перечислены в YAML-блоке `available_widgets` под полем `name` у каждого intent-а в методичке.
   - **ИМЯ ИНСТРУМЕНТА И ИМЯ INTENT-А — ЭТО РАЗНЫЕ ВЕЩИ!** Не подставляйте имя инструмента в поле `intent`. Например, если в методичке в `available_widgets` нет ни одного intent-а, релевантного вопросу — оставьте `planned_widgets` пустым. Лучше пустой список, чем неверный intent.
...
4. Для кросс-доменных запросов интегрируйте знания из всех релевантных методичек в ЕДИНЫЙ связный нарратив.
5. План должен покрывать: порядок сбора данных -> логику анализа -> выбор виджетов -> форму финального ответа.
6. Выбирайте widget intent внимательно из YAML-объявлений. Имя intent-а должно СОВПАДАТЬ с тем, что написано в `available_widgets[].intents[].name`.
...
9. УСЛОВНЫЕ/ЗАВИСИМЫЕ ИНСТРУМЕНТЫ — СОХРАНЯЙТЕ ИХ. Если методичка описывает инструмент Б, который вызывается только при выполнении некоторого условия в данных, полученных от другого инструмента А, вы ОБЯЗАНЫ:
   - включить инструмент Б в planned_tools вместе с инструментом А (не выбрасывайте инструмент Б как "нерелевантный");
...
class SkillConcentrator:
    ...
    async def concentrate(
        ...
        print(f"[Concentrator] Сжатие: {orig_size} -> {new_size} символов ({ratio:.0f}% от оригинала)")
        ...
        print(f"[Concentrator] Концентрат: "
              f"{parsed.get('concentrated_knowledge', '').strip()}")
